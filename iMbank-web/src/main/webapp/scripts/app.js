...
//}).
	...
	window.onbeforeunload = function (e) {
		...
		if (e) {
		    e.returnValue = 'Refreshing Page will Make BIO unfunctional, Proceed?';
		}

		// For Safari
		return 'Refreshing Page will Make BIO unfunctional, Proceed??';*/
	};
...
//	};
    ...
    angular.module("app", ["ngRoute",
            ...
            return $routeProvider.when("/", {
                ...
                templateUrl: "views/user/locked_users.html"  
            }).when("/user/rights", {
                templateUrl: "views/user/rights.html"
            }).when("/dashboard", {
                templateUrl: "views/dashboard.html"
            }).when("/reason", {
                templateUrl: "dialogs/reason.html"
            }).when("/masters/branches", {
                templateUrl: "views/masters/branch.html"
            }).when("/masters/printer", {
                templateUrl: "views/masters/printer.html"
            }).when("/masters/cardtype", {
                templateUrl: "views/masters/cardtype.html"
            }).when("/masters/printertype", {
                templateUrl: "views/masters/printertype.html"  	
            }).when("/cardprint/inquiry", {
                templateUrl: "views/cardprint/cardprint.html"
            }).when("/reports/branchcards", {
                templateUrl: "views/reports/cards_by_branch.html"
            }).when("/reports/printedcards", {
                templateUrl: "views/reports/cards_by_date.html"
            }).when("/reports/usercards", {
                templateUrl: "views/reports/cards_by_user.html"
            }).when("/reports/balancecards", {
                templateUrl: "views/reports/card_balance_report.html"
            }).when("/reports/rejectedcards", {
                templateUrl: "views/reports/rejected_cards.html"
            }).when("/cardinventory/allocatecards", {
                templateUrl: "views/cardinventory/allocate_card.html"
            }).when("/cardinventory/receivecards", {
                templateUrl: "views/cardinventory/receive_card.html"
            }).when("/cardinventory/rejectcards", {
                templateUrl: "views/cardinventory/qa_reject.html"
            }).when("/cardinventory/cardbalance", {
                templateUrl: "views/cardinventory/card_balance.html"
            }).when("/404", {
                templateUrl: "views/others/404.html"
            }).otherwise({
                redirectTo: "/404"
            })
        }]).run(function ($chkNotNullUndefined, $rootScope, localStorageService, $location, $filter, logger,$interval) {
        	 var idletime;
             var resettime;
        	...
        	lastDigestRun = Date.now(); 
                if ($chkNotNullUndefined(localStorageService.get('rxr')) == false) {
                    ...
                    if (!(next.templateUrl == "views/user/signin.html")) {
                        $location.path("/login");
                    }
                }
                else {
                    if (next.templateUrl == "views/user/signin.html") {
                        ...
                        if (next.templateUrl == "views/user/lock-screen.html") {
                            localStorageService.clearAll();
                        }
                        else {
                            if (!($location.path().toLowerCase() == "/dashboard"
                            	...
                            	if ($chkNotNullUndefined(next.templateUrl)) {
                            		//console.log($rootScope.UsrRghts.rightsHeaderList);
                            		//console.log("#" + $location.path());
...
//   - See more at: http://www.oodlestechnologies.com/blogs/AngularJS-caching-issue-for-Internet-Explorer#sthash.x2gq5QsC.dpuf
